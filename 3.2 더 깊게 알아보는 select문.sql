...
-- 실습 14-2. 14-1은 그룹 아이디 별로 총 구매한 금액을 조회해서 보여줍니다.
-- 			 만약 그룹 아이디 별로 총 구매한 금액이 1000 이상이면 사은품을 증정하려고 합니다.
-- 			 그룹 아이디 별로 총 구매한 금액이 1000 이상인 그룹의 아이디와 총 구매 금액을 조회합니다.
select
	mem_id as '그룹 아이디',
    sum(price * amount) as '총 구매 금액'
from buy
...
-- 실습 14-3. 14-2에서 조회된 데이터를 기준으로 사은품을 증정하려고 하는데,
-- 			 사은품은 순위에 따라 물품이 다를 수 있기 때문에
-- 			 총 구매 금액이 큰 순서대로 정렬하여 데이터를 조회합니다.
select
	mem_id as '그룹 아이디',
	sum(price * amount) as '총 구매 금액'
from buy
